...
def test_execution_timeout_config(tmp_path):
    ...
    executor = load_executor("basic", db)
    result = executor.run_and_cache(timeout=20)
    assert result == {
        "succeeded": [
            os.path.join(NB_PATH, "complex_outputs_unrun.ipynb"),
        ],
        "excepted": [],
        "errored": [],
    }
...
def test_execution_timeout_metadata(tmp_path):
    ...
    db = JupyterCacheBase(str(tmp_path))
    db.stage_notebook_file(path=os.path.join(NB_PATH, "complex_outputs_unrun_timeout.ipynb"))
    executor = load_executor("basic", db)
    result = executor.run_and_cache()
    assert result == {
        "succeeded": [],
