...
class OrmKernel(OrmBase):
    ...
    @staticmethod
    def from_nbformat(nb: nbformat.NotebookNode, doc_pk: int, doc_order: int):
        # TODO kernel_info, name
        kernel_type = nb.metadata.kernelspec.name
        orm_kernel = OrmKernel(
            kernel_type=kernel_type,
            meta_data=nb.metadata,
            doc_pk=doc_pk,
            doc_order=doc_order,
        )
        orm_kernel.codecells = [
            OrmCodeCell.from_nbformat(
                cell, doc_pk=doc_pk, doc_order=i, exec_order=i, kernel_pk=None
            )
...
class OrmCodeCell(OrmBase):
    ...
    def from_nbformat(
        ...
        if not cell.outputs:
            return orm_cell
        orm_cell.execution = OrmCellExecution(execution_count=cell.execution_count)
        orm_cell.execution.outputs = [
            OrmOutput.from_nbformat(output, order=i)
